...
import { makeInMemoryStore, InMemoryNegotiationStore, InMemoryTransferStore } from '../helpers/in-memory-store';
import { NegotiationState, ContractNegotiation } from '../../src/types/negotiation';
import { TransferState, TransferProcess } from '../../src/types/transfer';
import { ConsumerNegotiationHooks, ConsumerTransferHooks } from '../../src/types/hooks';
...
function makeNegotiationApp(hooks?: ConsumerNegotiationHooks) {
  ...
  const router = makeConsumerNegotiationRouter({ store: store.negotiation, hooks }, noopAuth);
  const app = express();
...
function makeTransferApp(hooks?: ConsumerTransferHooks) {
  const store = makeInMemoryStore();
  const router = makeConsumerTransferRouter({ store: store.transfer, hooks }, noopAuth);
  const app = express();
  app.use(express.json());
...
describe('Consumer — negotiation hooks', () => {
    ...
    it('is called with state=OFFERED and the offer terms', async () => {
      ...
      const res = await request(app)
        ...
        .send({
          ...
          callbackAddress: 'https://provider.example/dsp',
          offer: { '@id': 'urn:offer:1', target: 'urn:dataset:42', permission: [{ action: 'use' }] },
        });
      ...
      expect(received.offer).toMatchObject({ '@id': 'urn:offer:1', target: 'urn:dataset:42' });
    });
  });
  ...
  describe('onOfferReceived — POST /negotiations/:consumerPid/offers (counter-offer)', () => {
    it('is called with state=OFFERED and updated offer terms', async () => {
      const hook = jest.fn();
      const { app, negStore } = makeNegotiationApp({ onOfferReceived: hook });
      ...
      const res = await request(app)
        ...
        .send({
          ...
          providerPid: 'urn:uuid:prov-001',
          callbackAddress: 'https://provider.example/dsp',
          offer: { '@id': 'urn:offer:revised', target: 'urn:dataset:42', permission: [] },
        });
  ...
  describe('onAgreementReceived — POST /negotiations/:consumerPid/agreement', () => {
    it('is called with state=AGREED and the agreement attached', async () => {
      const hook = jest.fn();
      const { app, negStore } = makeNegotiationApp({ onAgreementReceived: hook });
  ...
  describe('onNegotiationFinalized — POST /negotiations/:consumerPid/events', () => {
    it('is called with state=FINALIZED', async () => {
      const hook = jest.fn();
      const { app, negStore } = makeNegotiationApp({ onNegotiationFinalized: hook });
      ...
      expect(res.status).toBe(200);
      expect(hook).toHaveBeenCalledTimes(1);
      expect(hook.mock.calls[0][0].state).toBe(NegotiationState.FINALIZED);
    });
  });
  ...
  describe('onNegotiationTerminated — POST /negotiations/:consumerPid/termination', () => {
    it('is called with state=TERMINATED', async () => {
      const hook = jest.fn();
      const { app, negStore } = makeNegotiationApp({ onNegotiationTerminated: hook });
      ...
      expect(res.status).toBe(200);
      expect(hook).toHaveBeenCalledTimes(1);
      expect(hook.mock.calls[0][0].state).toBe(NegotiationState.TERMINATED);
    });
  });

  describe('hook resilience', () => {
    it('a throwing hook does not affect the HTTP response', async () => {
      const throwingHook = jest.fn().mockRejectedValue(new Error('business logic exploded'));
      const { app, negStore } = makeNegotiationApp({ onNegotiationFinalized: throwingHook });
      seedNegotiation(negStore, { state: NegotiationState.VERIFIED });
...
describe('Consumer — transfer hooks', () => {
  describe('onTransferStarted — POST /transfers/:consumerPid/start', () => {
    it('is called with state=STARTED and the dataAddress', async () => {
      const hook = jest.fn();
      const { app, xferStore } = makeTransferApp({ onTransferStarted: hook });
      ...
      expect(received.dataAddress).toMatchObject({ endpoint: 'https://provider.example/data/42' });
    });
  });
  ...
  describe('onTransferCompleted — POST /transfers/:consumerPid/completion', () => {
    it('is called with state=COMPLETED', async () => {
      const hook = jest.fn();
      const { app, xferStore } = makeTransferApp({ onTransferCompleted: hook });
  ...
  describe('onTransferSuspended — POST /transfers/:consumerPid/suspension', () => {
    it('is called with state=SUSPENDED', async () => {
      const hook = jest.fn();
      const { app, xferStore } = makeTransferApp({ onTransferSuspended: hook });
      seedTransfer(xferStore, { state: TransferState.STARTED });
  ...
  describe('onTransferTerminated — POST /transfers/:consumerPid/termination', () => {
    it('is called with state=TERMINATED', async () => {
      const hook = jest.fn();
      const { app, xferStore } = makeTransferApp({ onTransferTerminated: hook });
      seedTransfer(xferStore, { state: TransferState.STARTED });
  ...
  describe('hook resilience', () => {
    it('a throwing hook does not affect the HTTP response', async () => {
      const throwingHook = jest.fn().mockRejectedValue(new Error('pipeline failed'));
      const { app, xferStore } = makeTransferApp({ onTransferStarted: throwingHook });
      seedTransfer(xferStore, { state: TransferState.REQUESTED });
