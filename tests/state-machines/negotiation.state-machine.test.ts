...
describe('Negotiation State Machine', () => {
  ...
  describe('isValidNegotiationTransition()', () => {
    it('allows CONSUMER to send ContractRequestMessage on null (new negotiation)', () => {
      expect(isValidNegotiationTransition(null, 'ContractRequestMessage', 'CONSUMER')).toBe(true);
    });

    it('rejects PROVIDER sending ContractRequestMessage on null', () => {
      expect(isValidNegotiationTransition(null, 'ContractRequestMessage', 'PROVIDER')).toBe(false);
    });

    it('allows PROVIDER to send ContractOfferMessage on null (provider-initiated)', () => {
      expect(isValidNegotiationTransition(null, 'ContractOfferMessage', 'PROVIDER')).toBe(true);
    });

    it('allows CONSUMER to send ContractRequestMessage on OFFERED (counter-offer)', () => {
      expect(
        isValidNegotiationTransition(NegotiationState.OFFERED, 'ContractRequestMessage', 'CONSUMER')
      ).toBe(true);
    });

    it('rejects PROVIDER sending ContractRequestMessage on OFFERED', () => {
      expect(
        isValidNegotiationTransition(NegotiationState.OFFERED, 'ContractRequestMessage', 'PROVIDER')
      ).toBe(false);
    });

    it('allows PROVIDER to counter-offer on REQUESTED', () => {
      expect(
        isValidNegotiationTransition(NegotiationState.REQUESTED, 'ContractOfferMessage', 'PROVIDER')
      ).toBe(true);
    });
  ...
  describe('nextNegotiationState() — valid transitions', () => {
    it('returns REQUESTED when Consumer initiates', () => {
      expect(nextNegotiationState(null, 'ContractRequestMessage', 'CONSUMER')).toBe(
        NegotiationState.REQUESTED
      );
    });
    ...
    it('returns OFFERED when Provider counter-offers on REQUESTED', () => {
      expect(
        nextNegotiationState(NegotiationState.REQUESTED, 'ContractOfferMessage', 'PROVIDER')
    ...
    it('returns AGREED when provider sends agreement', () => {
      expect(
        nextNegotiationState(NegotiationState.ACCEPTED, 'ContractAgreementMessage', 'PROVIDER')
  ...
  describe('nextNegotiationState() — invalid transitions', () => {
    it('throws InvalidNegotiationTransitionError on bad transition', () => {
      expect(() =>
        nextNegotiationState(null, 'ContractAgreementMessage', 'PROVIDER')
