...
export function makeConsumerNegotiationHandlers(deps: ConsumerNegotiationHandlerDeps) {
  /**
   ...
   */
  async function receiveInitialOffer(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const body = req.body as ContractOfferMessage;

      let nextState: NegotiationState;
      try {
        nextState = nextNegotiationState(null, 'ContractOfferMessage', 'PROVIDER');
      } catch (err) {
  ...
  async function receiveOffer(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      let nextState: NegotiationState;
      try {
        nextState = nextNegotiationState(negotiation.state, 'ContractOfferMessage', 'PROVIDER');
      } catch (err) {
        if (err instanceof InvalidNegotiationTransitionError) { badTransition(res, err); return; }
        throw err;
      }
   ...
   */
  async function receiveAgreement(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      let nextState: NegotiationState;
      try {
        nextState = nextNegotiationState(negotiation.state, 'ContractAgreementMessage', 'PROVIDER');
      } catch (err) {
        if (err instanceof InvalidNegotiationTransitionError) { badTransition(res, err); return; }
        throw err;
      }
   ...
   */
  async function receiveEvent(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const msgType: NegotiationMessageType = 'ContractNegotiationEventMessage:FINALIZED';

      let nextState: NegotiationState;
      try {
        nextState = nextNegotiationState(negotiation.state, msgType, 'PROVIDER');
      } catch (err) {
        if (err instanceof InvalidNegotiationTransitionError) { badTransition(res, err); return; }
        throw err;
      }
