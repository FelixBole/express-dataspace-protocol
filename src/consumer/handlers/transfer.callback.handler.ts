...
export function makeConsumerTransferHandlers(deps: ConsumerTransferHandlerDeps) {
  /**
   * POST /transfers/:consumerPid/start — §10.3.2
   * Provider signals transfer has started (PULL: includes dataAddress).
   */
  async function receiveStart(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const body = req.body as TransferStartMessage;
      const transfer = await deps.store.findByConsumerPid(req.params.consumerPid);
      if (!transfer) { notFound(res, req.params.consumerPid); return; }

      let nextState: TransferState;
      try {
        nextState = nextTransferState(transfer.state, 'TransferStartMessage', 'PROVIDER');
      } catch (err) {
   ...
   */
  async function receiveCompletion(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const transfer = await deps.store.findByConsumerPid(req.params.consumerPid);
      if (!transfer) { notFound(res, req.params.consumerPid); return; }
      ...
      let nextState: TransferState;
      try {
        nextState = nextTransferState(transfer.state, 'TransferCompletionMessage', 'PROVIDER');
      } catch (err) {
        if (err instanceof InvalidTransferTransitionError) { badTransition(res, err); return; }
        throw err;
      }
   ...
   */
  async function receiveTermination(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      let nextState: TransferState;
      try {
        nextState = nextTransferState(transfer.state, 'TransferTerminationMessage', 'PROVIDER');
      } catch (err) {
        if (err instanceof InvalidTransferTransitionError) { badTransition(res, err); return; }
        throw err;
      }
   ...
   */
  async function receiveSuspension(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      let nextState: TransferState;
      try {
        nextState = nextTransferState(transfer.state, 'TransferSuspensionMessage', 'PROVIDER');
      } catch (err) {
        if (err instanceof InvalidTransferTransitionError) { badTransition(res, err); return; }
        throw err;
      }
  ...
  return { receiveStart, receiveCompletion, receiveTermination, receiveSuspension };
}
