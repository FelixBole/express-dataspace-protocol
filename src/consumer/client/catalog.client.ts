...
export function makeCatalogClient(deps: CatalogClientDeps) {
   ...
   */
  async function getDataset(providerBaseUrl: string, datasetId: string): Promise<Dataset> {
    return dspFetch<Dataset>(
      buildUrl(providerBaseUrl, `/catalog/datasets/${encodeURIComponent(datasetId)}`),
      { method: 'GET' },
      deps.getOutboundToken
    );
  }
