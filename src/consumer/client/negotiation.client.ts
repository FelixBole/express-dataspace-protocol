...
export function makeNegotiationClient(deps: NegotiationClientDeps) {
	...
	async function counterOffer(
		...
		await dspPost(
			buildUrl(
				providerBaseUrl,
				`/negotiations/${encodeURIComponent(providerPid)}/request`,
			),
			msg,
			deps.getOutboundToken,
		);
		...
		if (deps.store) {
			await deps.store.update(providerPid, {
				state: NegotiationState.REQUESTED,
				offer,
			});
		}
	...
	async function acceptNegotiation(
		...
		await dspPost(
			buildUrl(
				providerBaseUrl,
				`/negotiations/${encodeURIComponent(providerPid)}/events`,
			),
			{
				"@context": [DSP_CONTEXT],
	...
	async function terminateNegotiation(
		...
		await dspPost(
			buildUrl(
				providerBaseUrl,
				`/negotiations/${encodeURIComponent(providerPid)}/termination`,
			),
			msg,
			deps.getOutboundToken,
		);
