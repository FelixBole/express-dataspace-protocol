...
export function makeTransferClient(deps: TransferClientDeps) {
	...
	async function completeTransfer(
		...
		await dspPost(
			buildUrl(
				providerBaseUrl,
				`/transfers/${encodeURIComponent(providerPid)}/completion`,
			),
			{
				"@context": [DSP_CONTEXT],
	...
	async function suspendTransfer(
		...
		await dspPost(
			buildUrl(
				providerBaseUrl,
				`/transfers/${encodeURIComponent(providerPid)}/suspension`,
			),
			msg,
			deps.getOutboundToken,
		);
	...
	async function terminateTransfer(
		...
		await dspPost(
			buildUrl(
				providerBaseUrl,
				`/transfers/${encodeURIComponent(providerPid)}/termination`,
			),
			msg,
			deps.getOutboundToken,
		);
