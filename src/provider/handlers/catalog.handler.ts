...
type CatalogFilterFn = (filter: unknown, store: CatalogStore) => Promise<import('../../types/catalog').Catalog>;
...
export function makeCatalogHandlers(deps: CatalogHandlerDeps) {
   ...
   */
  async function requestCatalog(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (body.filter !== undefined) {
        if (!deps.catalogFilter) {
          res.status(400).json({
            ...
            reason: ['This Catalog Service does not support filter expressions.'],
          });
          return;
        }
        const catalog = await deps.catalogFilter(body.filter, deps.store);
   ...
   */
  async function getDataset(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id } = req.params;
