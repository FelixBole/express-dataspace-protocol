...
export function makeTransferHandlers(deps: TransferHandlerDeps) {
	...
	async function getTransferProcess(
		...
		try {
			const transfer = await deps.store.findByProviderPid(
	...
	async function providerStartTransfer(
		...
		if (!transfer.callbackAddress)
			throw new Error(
				`Transfer '${providerPid}' has no callbackAddress.`,
			);

		const nextState = nextTransferState(
			transfer.state,
			"TransferStartMessage",
			"PROVIDER",
		);
		...
		const msg: TransferStartMessage = {
			...
			...(updated.dataAddress
				? { dataAddress: updated.dataAddress }
				: {}),
		};

		await providerPost(
			buildUrl(
				transfer.callbackAddress,
				`/transfers/${encodeURIComponent(updated.consumerPid)}/start`,
			),
			msg,
	 ...
	 */
	async function providerCompleteTransfer(
		providerPid: string,
	): Promise<TransferProcess> {
		const transfer = await deps.store.findByProviderPid(providerPid);
		if (!transfer)
		...
		await providerPost(
			buildUrl(
				transfer.callbackAddress,
				`/transfers/${encodeURIComponent(updated.consumerPid)}/completion`,
			),
			msg,
			deps.getOutboundToken,
		);
	...
	async function providerSuspendTransfer(
		...
		await providerPost(
			buildUrl(
				transfer.callbackAddress,
				`/transfers/${encodeURIComponent(updated.consumerPid)}/suspension`,
			),
			msg,
			deps.getOutboundToken,
		);
	...
	async function providerTerminateTransfer(
		...
		await providerPost(
			buildUrl(
				transfer.callbackAddress,
				`/transfers/${encodeURIComponent(updated.consumerPid)}/termination`,
			),
			msg,
			deps.getOutboundToken,
		);
