...
import { makeTransferHandlers, TransferHandlerDeps } from '../handlers/transfer.handler';

export function makeTransferRouter(deps: TransferHandlerDeps, auth: RequestHandler): Router {
  const router = Router();
  const {
    getTransferProcess,
    requestTransfer,
