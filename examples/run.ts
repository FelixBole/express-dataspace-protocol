...
import { createDspProvider, createDspConsumer, createDiskStore } from "../src";
...
const log = {
	...
	consumer: (msg: string) =>
		console.log(
			`${DIM}${ts()}${R}  ${YELLOW}${BOLD}[CONSUMER]${R}  ${msg}`,
		),
	runner: (msg: string) =>
		console.log(
			`${DIM}${ts()}${R}  ${MAGENTA}${BOLD}[ RUNNER ]${R}  ${msg}`,
		),
};
...
async function main() {
	...
	const providerStore = await createDiskStore({
		dir: path.join(TMP, "provider"),
	});
	const consumerStore = await createDiskStore({
		dir: path.join(TMP, "consumer"),
	});
		...
		dataset: [
			{
				...
				description:
					"Monthly temperature averages for 2024 across 195 countries.",
				hasPolicy: [
					{
		...
		hooks: {
			negotiation: {
				...
				onNegotiationRequested: async (negotiation) => {
					...
					await provider.negotiation.sendCounterOffer(
				...
				onNegotiationAccepted: async (negotiation) => {
					log.provider(
						`Consumer accepted offer  providerPid=${negotiation.providerPid}`,
					);
				...
				onAgreementVerified: async (negotiation) => {
					log.provider(
						`Agreement verified  providerPid=${negotiation.providerPid}`,
					);
			...
			transfer: {
				...
				onTransferRequested: async (transfer) => {
					...
					log.provider(
						"  → Provisioning HTTP_PULL data channel (simulated 150 ms)...",
					);

					await new Promise((r) => setTimeout(r, 150));

					await provider.transfer.providerStartTransfer(
						transfer.providerPid,
						{
							"@type": "DataAddress",
							endpointType: "https://w3id.org/idsa/v4.1/HTTP",
							endpoint:
								"https://data.example.com/climate-2024.csv",
							endpointProperties: [
								{
		...
		hooks: {
			negotiation: {
				...
				onOfferReceived: async (negotiation) => {
					log.consumer(
						`Offer received  consumerPid=${negotiation.consumerPid}`,
					);
					log.consumer(
						`  Terms: ${JSON.stringify(negotiation.offer?.permission)}`,
					);
			...
			transfer: {
				...
				onTransferStarted: async (transfer) => {
					log.consumer(
						`Transfer STARTED ✓  consumerPid=${transfer.consumerPid}`,
					);

					const addr = transfer.dataAddress as
						| Record<string, unknown>
						| undefined;
					if (addr) {
						log.consumer(`  Endpoint : ${addr["endpoint"]}`);
						const props = addr["endpointProperties"] as
							| Array<Record<string, string>>
							| undefined;
						const authProp = props?.find(
							(p) => p["name"] === "authorization",
						);
						if (authProp)
							log.consumer(`  Auth     : ${authProp["value"]}`);
					}
	...
	const negotiation = await consumer.negotiation.requestNegotiation(
		PROVIDER_BASE,
		{
			callbackAddress: CONSUMER_CALLBACK,
			offer: {
	...
	console.log("\n" + "═".repeat(66));
	console.log(
		`${GREEN}${BOLD}   ✓  Complete DSP flow finished successfully!${R}`,
	);
	console.log("");
